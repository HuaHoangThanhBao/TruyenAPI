...
namespace API.Extensions
{
    public class APICredentialAuth
    {
        ...
        public static ResponseDetails JWTKeyCheck(string key)
        {
            var fileName = Path.Combine("Resources", "Keys", "credential.json");
            var path = Path.Combine(Directory.GetCurrentDirectory(), fileName);
            ...
            var json = File.ReadAllText(path);
            dynamic jsonObj = JsonConvert.DeserializeObject(json);
            var keyObj = jsonObj["jwt_key"];

            if (key == Convert.ToString(keyObj))
                return new ResponseDetails() { StatusCode = ResponseCode.Success, Message = "JWT key hợp lệ" };
            else
                return new ResponseDetails() { StatusCode = ResponseCode.Error, Message = "JWT key không hợp lệ" };
        }


        public static ResponseDetails GetJWTKey()
        {
            var fileName = Path.Combine("Resources", "Keys", "credential.json");
            var path = Path.Combine(Directory.GetCurrentDirectory(), fileName);
            ...
            var json = File.ReadAllText(path);
            dynamic jsonObj = JsonConvert.DeserializeObject(json);
            var keyObj = Convert.ToString(jsonObj["jwt_key"]);

            return new ResponseDetails() { StatusCode = ResponseCode.Success, Value = keyObj, Message = "Lấy JWT key thành công" };
        }
    }
}
